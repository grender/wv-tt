...
class ExchangeTest extends FlatSpec with MustMatchers {
  ...
  def runOnExchange[T[_]](exchange: AbstractExchange[T],
                          ...
                          toList: T[Order] => List[Order]): Unit = {
    val exchangeName = exchange.getClass.getSimpleName

    s"$exchangeName foundSellOrder" should "correct found order and return" in {
      val inputAssets = Map(
        "C1" -> assets(usd = 30, shareA = 2),
        "C2" -> assets(usd = 5, shareA = 5)
      )
    ...
    s"$exchangeName updateState" should "correct found sell order for buy order and apply to assets and get " in {
      val buyOrder = order(
      ...
      val processingStep = exchange.createProcessingStepState(
        ProcessingStepState[List](clientAssets = inputAssets,
